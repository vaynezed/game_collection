...
private:
    ...
    constexpr static int port { 12345 };
...
public:
    ...
    static constexpr int PAIR_MODEL { 1 }, AI_MODEL { 0 }, NET_MODEL { 2 };
    ...
    void game_model_web(int x_pos, int y_pos);
...
public:
    AIOpponent(Chess* ptr, bool black_player)
        : Opponent(ptr)
...
class PairOpponent : public Opponent {
public:
    PairOpponent(Chess* ptr)
        : Opponent(ptr)
    {
    }
    void move() override;
...
class WebOpponent : public Opponent {
public:
    WebOpponent(Chess* ptr, int room_id, std::shared_ptr<Server> server);
    ...
    struct EnterRoomRsp enter_room();

    struct LeaveRoomRsp {
        bool code;
        std::string message;
    };
    struct LeaveRoomRsp leave_room();
    void move() override;
    void set_black_player(bool black_player);
    bool is_black_player();
    void self_move(struct Pos& source_pos, struct Pos& target_pos);
